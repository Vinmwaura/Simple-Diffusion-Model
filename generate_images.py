...
if __name__ == "__main__":
    ...
    parser.add_argument("--min_channel", help = "U Net min channels used. (default: 64).", default=64, type=int)
    ...
    parser.add_argument("--attn_layers", help = "U Net layers using attention. (default: 3, 4).", nargs='*', default=[3, 4], type=int)
    ...
    try:
        ...
        with torch.no_grad():
            if args["diff_alg"] == DiffusionAlg.DDPM.name.lower():
                ...
                for noise_step in range(args["max_T"], 20, -20):
                    # t: Time Step
                    t = torch.tensor([noise_step], device=args["device"])
