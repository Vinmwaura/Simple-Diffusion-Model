...
def generate_sr_images_diffusion(
    ...
    parser.add_argument(
        ...
        "--labels",
        nargs="*",
        help="Labels Conditional e.g One-hot encoding.",
        type=float,
        default=None)
    parser.add_argument(
        "--lr_img_path",
        help="File path to image be upsampled.",
        type=pathlib.Path,
        default=None)
    parser.add_argument(
        "-T",
    ...
    if lr_img is not None:
        ...
        if lr_img_path is None or not os.path.isfile(lr_img_path) or imghdr.what(lr_img_path) not in SUPPORTED_IMG_FORMATS:
            raise ValueError("Invalid/No path for low resolution image or unsupported image.")
    ...
    for model_dict in models_details["models"]:
        ...
        if noise is None:
            img_num, _, H_lr, W_lr = lr_img.shape
        ...
        x0_approx_delta = cold_diffusion_sampling(
            ...
            cond_img=x_t_cond_input,
            labels_tensor=labels_tensor,
            skip_step_size=args["cold_step_size"],
            device=args["device"],
            log=log)

    # Save Image from Super-Resolution Diffusion.
    x0_approx = x0_approx_upsample + x0_approx_delta
    if save_locally:
